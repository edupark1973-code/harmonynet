...
| **FR-02** | **GNB** | 하모니넷 메뉴 네비게이션 | 기존 사이트 메뉴 순서인 HUSS소식, 로컬소식, 영상뉴스, 콘텐츠기업소개, 스타트업 뉴스, 민간자격과정을 유지하고 로그인/기사작성 링크 제공 | **P0** |
| **FR-03** | **GNB** | 모바일 네비게이션 | 모바일 해상도에서 `전체 메뉴` 펼침 UI로 모든 내부·외부 메뉴를 잘림 없이 제공 | **P0** |
...
| **FR-07** | **상세** | 기사 본문 상세 렌더링 | 고유 기사 ID로 컬렉션 API(`posts?include={id}`)를 조회해 제목, 작성일, 카테고리, 대표 이미지와 본문을 정확히 렌더링 | **P0** |
...
5. **Cafe24 WAF와 데이터 조회 경로**:
   * Firebase App Hosting 서버 IP에서 WordPress REST API를 호출하면 Cafe24 WAF가 JSON 대신 차단 HTML을 반환할 수 있다.
   * 공개 콘텐츠인 기사, 카테고리, 검색, 고정 페이지는 브라우저에서 WordPress REST API를 직접 조회한다. WordPress의 CORS 정책 변경 시 재검증한다.
   * 존재하지 않는 기사 ID를 최신 기사로 대체하지 않고 명확한 오류 화면을 표시한다.
