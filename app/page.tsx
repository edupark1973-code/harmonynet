...
const WP_DIRECT_URL = 'https://huss.harmonynet.kr/wp-json/wp/v2/posts?_embed&per_page=13';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    async function loadClientPosts() {
      try {
        setLoading(true);
        // 브라우저에서 직접 카페24 WP REST API를 호출하여 해외 IP 차단 우회
        ...
        if (isMounted) {
          ...
          setError(`실시간 데이터 로딩 중 오류가 발생했습니다 (${message}).`);
        }
      } finally {
        if (isMounted) {
          setLoading(false);
        ...
        {loading ? (
          <div className="space-y-8 animate-pulse">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
              <div className="h-96 rounded-lg bg-neutral-200 dark:bg-neutral-800 lg:col-span-8"></div>
              <div className="space-y-4 lg:col-span-4">
                <div className="h-24 rounded bg-neutral-200 dark:bg-neutral-800"></div>
                <div className="h-24 rounded bg-neutral-200 dark:bg-neutral-800"></div>
                <div className="h-24 rounded bg-neutral-200 dark:bg-neutral-800"></div>
              </div>
            </div>
